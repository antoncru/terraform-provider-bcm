...
// deviceImportStateVerifyIgnore is the base set of attributes to ignore during ImportStateVerify
// for bcm_cmdevice_device. These are fields that BCM populates with defaults, computes
// server-side, or returns in a format that differs from the Terraform config (e.g. "CATEGORY").
// Tests that also need to ignore management_network should use deviceImportIgnoreWithMgmtNetwork.
var deviceImportStateVerifyIgnore = []string{
	"force",
	"boot_loader",
	...
	"install_boot_record", "install_mode", "next_boot_install_mode",
	"node_installer_disk", "pxelabel", "raidconf", "version_config_files",
	"cpuspeed_governor", "io_scheduler", "kernel_output_console", "kernel_version",
	"custom_ping_script", "custom_ping_script_argument",
	"custom_power_script", "custom_power_script_argument",
	"custom_remote_console_script", "custom_remote_console_script_argument",
	"finalize", "initialize",
	"exclude_list_full", "exclude_list_grab", "exclude_list_grabnew",
	"exclude_list_manipulate_script", "exclude_list_sync", "exclude_list_update",
	"data_node", "disable_fabric_nvme", "force_full_environment",
	"supports_gnss", "template_node",
	"tag", "use_exclusively_for", "userdefined1", "userdefined2",
	"rack", "software_image_proxy",
	"block_devices_cleared_on_next_boot.#", "modules.#",
	"bios_setup", "bmc_settings", "extra_values", "proxy_settings",
	"se_linux_settings", "time_zone_settings",
	"fsexports", "fsmounts", "gpu_settings", "power_distribution_units",
	"static_routes", "switch_ports", "user_defined_resources",
	"interfaces.#",
	"interfaces.0.%",
	...
	"interfaces.0.bring_up_during_install",
	"interfaces.0.gateway",
	"interfaces.0.lanchannel",
	"interfaces.0.on_network_priority",
	"interfaces.0.vlanid",
	"interfaces.0.alternative_hostname",
	"interfaces.0.connected_mode",
	"interfaces.0.ipv6_dhcp",
	"interfaces.0.speed",
	"interfaces.0.additional_hostnames.#",
}

// deviceImportIgnoreWithMgmtNetwork extends the base ignore list with management_network,
// used by most import tests where management_network is not under verification.
var deviceImportIgnoreWithMgmtNetwork = append(
	append([]string{}, deviceImportStateVerifyIgnore...),
	"management_network",
)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Step 2: Import — management_network should be verified (NOT ignored).
			{
				...
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateVerifyIgnore: deviceImportStateVerifyIgnore,
...
// power_control value is rejected by the schema validator.
...
`,
				...
				ExpectError: regexp.MustCompile(`must be none`),
			},
		},
	})
}
