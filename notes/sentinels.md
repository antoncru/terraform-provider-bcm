...
BCM's JSON-RPC API is strongly typed — every field must have a value of the declared type. Where Terraform would use `null` to mean "not set," BCM uses a **type-appropriate placeholder** called a sentinel value. These look like valid data but are a default value.
...
Each sentinel is intercepted during parsing and converted to `null` for internal Terraform processing:
...
| `dhcp` | `false` | BCM default for new interfaces |
