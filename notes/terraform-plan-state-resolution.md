...
| **API payload** | Delta — "what to change" | Only fields being set or modified | Varies per operation |
...
**The API payload is always minimal.** The `Set*Field` helpers omit null/unknown fields from the entity sent to BCM. On create, this lets BCM apply its own defaults. On update, this means only changed fields are sent — BCM keeps existing values for omitted keys (patch semantics).
...
When the user omits a field, Terraform accepts whatever BCM has — including external changes. Drift is only flagged when the user's HCL explicitly sets a value that conflicts with what BCM returned.

## Why Optional-Only Fields Need the Workaround (and Why It's Wrong)
...
BCM's C++ backend uses zero-equivalent placeholders instead of null for "not set":
...
The provider maps these to `types.StringNull()` on read so Terraform doesn't treat them as meaningful user values. These sentinels are stable — they're tied to BCM's data model types, not version-specific defaults.

This is distinct from **application defaults** like `mtu = 1500` or `domainName = "cluster.local"`. Those are real values that a user might intentionally set and that could change across BCM versions. The provider should not suppress them — it should store whatever BCM returns and let `Optional+Computed` handle the lifecycle.
